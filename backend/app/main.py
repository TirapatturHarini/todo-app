...
from .telemetry import (
    ...
    record_histogram_with_exemplar, record_counter_with_exemplar,create_exemplar_counter,create_exemplar_histogram,
...
# for status logging
class StatusEnsureFormatter(logging.Formatter):
    def format(self, record):
        # DEBUG: See what's happening with status
        original_status = getattr(record, 'status', None)
        
        # Ensure status is always present for Loki labels
        if not hasattr(record, 'status') or not record.status:
            record.status = 'unknown'
            
        # DEBUG: Print if status was overridden
        if hasattr(record, 'extra') and 'status' in record.extra:
            print(f"DEBUG: Record has extra status: {record.extra['status']}, record.status: {record.status}")
            record.status = record.extra['status']  # Force override
            
        return super().format(record)

def setup_logging():
    """Setup logging with consistent status labels for Loki"""
    ...
    formatter = StatusEnsureFormatter(
...
# Middleware to measure request time and add trace context
@app.middleware("http")
...
# 1. Enhanced create_todo with better operation tracking
...
# 3. Enhanced get_todos with better operation tracking
...
# 4. Enhanced update_todo with better operation tracking
...
# 5. Enhanced delete_todo with better operation tracking
...
# 6. Enhanced complete_todo with better operation tracking
