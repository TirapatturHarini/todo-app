...
from .telemetry import setup_telemetry, get_tracer, get_meter, add_business_labels
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s %(levelname)s [trace_id=%(otelTraceID)s span_id=%(otelSpanID)s status=%(status)s] %(message)s"
,
    handlers=[
...
async def create_todo(todo: TodoCreate, db: Session = Depends(get_db)):
    with tracer.start_as_current_span("create_todo"):
        ...
        add_business_labels({"status": "created to do successfully"}) 
        try:
...
async def get_todo(todo_id: int, db: Session = Depends(get_db)):
    with tracer.start_as_current_span("get_todo"):
        ...
        add_business_labels({"status": "listing todos"}) 
...
async def update_todo(todo_id: int, todo_update: TodoUpdate, db: Session = Depends(get_db)):
    with tracer.start_as_current_span("update_todo"):
        ...
        if todo_update.completed is not None:
            db_todo.completed = todo_update.completed
            add_business_labels({"status": "updating" if todo_update.completed else "pending"})
...
async def delete_todo(todo_id: int, db: Session = Depends(get_db)):
    with tracer.start_as_current_span("delete_todo"):
        ...
        try:
            ...
            add_business_labels({"status": "deleting todo..."}) 
...
async def metrics_info():
    add_business_labels({"status": "sending metrics"}) 
